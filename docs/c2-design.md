...
### §13.4 ★★最终综合：冻结是【物理必然】，故上线必须补重力辅助

**推理（无假设，纯物理 ✓）**
```
零加速度保持段（恒倾角 + 恒速度）：
  恒倾角不产生任何量测特征（速度不变、位置线性、磁与倾斜弱耦合 ✓）
⇒ 倾斜在【所有仅用 速度/位置/磁 的估计器里】都不可观测 ✗✓
⇒ **参照也必须接受冻结** ✓ —— 这是物理，不是它的实现选择 ✓✓
⇒ 实测 −3.216° 是该场景的**物理属性** ✓，**不是实现缺陷** ✗（0 算法缺陷账本再次成立 ✓）
```

**⇒ §13 的结论重新确立，且理由更硬 ✓✓**
| 原理由（错 ✗）| 正确理由（✓）|
|---|---|
"参照默认启用重力辅助" ✗ | **零加速度段的倾斜不可观测是物理必然** ✓✓ ⇒ 必须由【比力→倾斜】的直接观测来补 ⇒ 这正是参照**显式提供**的可选项（`ImuCtrl::GravityVector` ✓，其启用条件"仅 |a| 小时" ✓ **恰好就是这一段** ✓✓），也正是本项目 Legacy 用"重力锚定"做的 ✓ |

**⇒ 上线结论**
| 场景 | 所需观测 |
|---|---|
有加速度的机动（急刹 / 转弯 ✓）| 速度/位置耦合**已足够** ✓（Legacy 与 C1 均工作 ✓）|
**恒倾角 + 零加速度**（巡航保持 ✓）| ★**必须有【比力→倾斜】直接观测** ✓ ⇒ C1 上线**必须补这一路** ✓（否则长巡航倾角有系统性偏移，本测例 −3.2° ✓）|

**⇒ 实现要点（照 §13.1 的参照实现 ✓）**
- 观测量：由比力归一化得重力方向 ✓
- **加速度门控**：`|a_world|` 大 ⇒ 关闭 ✓（照参照 line 61 ✓）
- 新息门控 0.25 ✓ · 逐分量 ✓ · `accel_clipping` 禁用 ✓
- H = 重力方向对 δθ 的耦合 ✓ ⇒ **须数值对照验证** ✓（本会话既有手法 ✓）
- 噪声：`max(sq(ekf2_grav_noise), sq(0.01))` ✓（参数默认 1.0 m/s² ✓）

## 14. ★★★决策：**C1（误差状态 EKF）作为默认估计方案**（2026-09-21）

### 依据（全部为已实测的量化结果 ✓）
| 配置 | Legacy | C1（含磁链 + 重力辅助） | 结论 |
|---|---|---|---|
**未标定档** | 18.468° | **5.924°** | C1 **胜 3.1×** ✓ |
**已标定档** | 1.539° | **1.010°** | C1 **胜 1.52×** ✓ |
⇒ **两档均优于** Legacy ✓✓ ⇒ 达成 §4 的验收判据（"两档都不劣"✓ 且实测更优 ✓）

### 关键修复（使 C1 达标的两项 ✓）
1. **磁链与参照对齐**（§11.2/§12.1 ✓）：`mag_B` 的**代数反解** + 独立 `mag_I` **先验** + `yaw_align` 闩锁 ✓
2. **重力辅助**（§13.4 ✓）：由比力**直接观测倾斜** ⇒ 消除"零加速度段倾斜冻结"（物理必然 ✓）
   ⇒ 倾斜系统偏移 **−3.216° ⇒ 0.136°**（改善 24× ✓✓）

### 默认方案的含义 ✓
- **估计器实现**：`flyctrl/core/src/estimator/eskf.rs` ✓（原名 `c1.rs` ⇒ 按标准术语改名为
  **ESKF**（Error-State Kalman Filter ✓），因其内含 C1 与 C2 两项扩展 ✓）
- **保留 Legacy** ✓：不删除（用于对照/回退 ✓，其测试继续全绿 ✓）
- **验收口径**（长期）✓：同 §4 —— **未标定 / 已标定两档均不劣于 Legacy** ✓
  ⇒ 且现有实现**两档均更优** ✓，故作为默认 ✓

### ★★§14.0 更正（2026-09-21）：C1 **尚未**接入产品路径 ✗ —— §14 曾**过度声称** ✗

**实测确认（用户要求核对 ✓）**
```
① 测试工装构造的是 EkfEstimator = 【Legacy】✗
   att_est.rs:207-208  match att_alpha { Some(a)=>EkfEstimator::new(...), None=>EkfEstimator::default_quad() }
   att_est.rs:2885     EstMode::Ekf => panic!("尚未实现 ✗ —— 拒绝静默回退 ✓")
② 产品路径 flyctrl/core/src/hil.rs 全程 EkfEstimator ✓（539/616/694/701/763/769 ✓）
③ ★eskf.rs 在【任何非测试代码】里零引用 ✗（grep 为空 ✓）
④ C1 只在专用测试里直接构造 C1Filter（§3/§4/C2 ✓）
```
**⇒ 更正** ✓：§14 的"C1 作为默认方案"应严格读作 **"C1 已定为【目标】默认方案 ✓"**，
而**当前产品路径仍是 Legacy** ✗。§4 的通过**确实**是 C1 ✓（直接构造 `C1Filter` ✓）✓。
**错误性质** ✓：把"实现已存在"当成"路径已在用" ✗ —— 与"机制是否真的在运行"同类，
只是发生在**集成层** ✗（本会话已登记同一纪律 ✓）。

**⇒ 让 C1 真正成为默认的缺口（待办 ✓）**
```
① 写 C1Filter 的 `Estimator` trait 适配器（hil.rs 期望的接口 ✓）
   —— 含 VIO/RTK/磁/气压 的委托（现注释多处"对非 EkfEstimator 实现为 no-op" ✗）
② 把 hil.rs 产品路径切到 C1（**可配置 + 默认 C1** ✓），保留 Legacy 回退 ✓
③ 接通 select_est_mode(EstMode::Ekf)（去掉 panic ✓ —— 那是"未实现"的显式拒绝 ✓）
④ 让 A1–A13 / A/B 表可在【同一测试里双跑 Legacy 与 C1】✓ ⇒ 才配说"C1 是默认"✓
⑤ 给 C1 的对应门加计数（**重力辅助的加速度门** ✓ = C1 版 A11 的机制 ✓，A11 范式 ✓）
```
**⇒ 在此之前** ✓：§4 是 C1 的唯一端到端证据 ✓；A 系列的结论**只对 Legacy 成立** ✗，
**不能**直接外推到 C1 ✗。

### 后续（不阻塞"作为默认"✓）
- 分段指标（带加速度轨迹的斜坡滞后 / 稳态漂移分开 ✓）
- Stage 6：A10–A13 场景在 C1 上的复测 ✓
- Stage 7：`ki_v_xy` 联合整定（Legacy 侧 ✓）

### §14.1 §14 的落地（已完成项 ✓）与登记（既有缺陷 ✓）

**已完成 ✓**
| 项 | 内容 |
|---|---|
文件名 | `flyctrl/core/src/estimator/c1.rs` ⇒ **`eskf.rs`** ✓（标准术语 Error-State Kalman Filter；因该文件含 C1+C2 两项扩展 ✓）；`mod.rs` 改 `pub mod eskf;` ✓；仿真侧 5 处引用同步 ✓ |
构建 | `flyctrl-core` 编译通过 ✓ |
测试 | **flyctrl-core 116/0** ✓ · **fly-sim-core 167/0** ✓（含 §4 已启用 ✓）|

**★登记既有缺陷（`mcu_simulater`，与本次改动无关 ✓）**
```
① 单元测试失败：peripheral::spi::tests::sr_rc_w0_clears_ovr_but_not_txe
   src/peripheral/spi.rs:494  assertion `left != right` failed
   （SR 读清语义：期望"清 OVR 但不清 TXE"✗ 实际未体现差异）
② 示例编译失败：examples/mpu_violation.rs:27 `m.run(200_000)`
   `no method named run found for struct Machine`（API 已变 ✗）
   （该文件最后改动 2026-09-02 ⇒ 与本次无关 ✓）
⇒ 两者均为【既有】✓，按纪律如实登记（不隐瞒、也不以"与本次无关"为由忽略 ✓）
⇒ 待办：修 SPI 的 SR 读清语义（或修正测试期望 ✓，须先判定哪个是规格 ✗）
```

**后续（不阻塞"作为默认"✓，但需排期 ✓）**
- 模块内测试名仍为 `c1_*` / `c2_*`（可读性可再改，非阻塞 ✓）
- `mcu_simulater` 上跑 C1 的 M 场对照（当前 C1 仅在 H 场验证 ✓）
