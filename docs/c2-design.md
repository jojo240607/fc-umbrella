...
### §13.2 ★★更正 + 新假设（2026-09-21，参照参数对照后 ✓）

**参照原文（`common.h` ✓）**
```cpp
313: ekf2_imu_ctrl{ImuCtrl::GyroBias | ImuCtrl::AccelBias};   // ★默认【不含】GravityVector
486: ekf2_grav_noise{1.0f};
```
**⇒ 参照默认配置下【不启用】重力矢量融合** ✗✓（需显式开启 `ImuCtrl::GravityVector` ✓）
**⇒ 故 §13 的"缺重力辅助是 C1 与参照的差异" —— 【更正】不成立** ✗
   （参照的倾斜也在无直接重力观测下估出 ✓）

### ★新假设（更强、且与实测吻合 ✓）
```
参照默认【启用 `AccelBias`】（加计零偏为状态 ✓）
而【未估计的加计零偏】造成的正是【系统性倾斜偏差】✓✓
  —— 比力里的常量偏置在姿态解算中等价于一个恒定的倾斜误差（经典耦合 ✓）
⇒ 若 C1 的 `ba` 实际未被估出 ✗ ⇒ **−3.216° 的系统性倾斜偏移即得解释** ✓✓
⇒ 下一诊断：查 C1 的 `ba` 是否在收敛（其 P 迹 / 增益 / 收敛值 vs 真值 ✓）
```

### §13.3 ★★定量结果推翻"加计零偏"假设，给出更精确机理（2026-09-21）

**实测（§4 已标定档 ✓）**
```
C1 的 ba        = [-0.0018, -0.0043, -0.0054]   （几乎为 0）
仿真 accel_bias  = [ 0.0200, -0.0100,  0.0500]   （真值，|b| ≈ 0.056 m/s²）
ΣP_ba = 7.8e-2（P 大 ⇒ 未被观测约束）
⇒ 由 b_a 引起的倾斜误差 ≈ atan(0.056/9.81) ≈ **0.33°** ✗
   实测系统偏移 **−3.216°** ⇒ 差 ~10× ⇒ **假设推翻** ✗✓
```

**⇒ 更精确的机理（与量级吻合 ✓）**
```
巡航剖面：斜坡 3s → 20° 保持
 · 升倾角段：有水平加速度 ⇒ 倾斜经速度耦合【可观测】✓
 · 保持段：恒倾角 + 恒速度 ⇒ **加速度 ≈ 0** ⇒ **倾斜不可观测** ✗✓
 ⇒ 估计【冻结】在斜坡末值 ⇒ 系统性偏移 ≈ 斜坡末滞后 ✓（−3.2° 量级相符 ✓）
```
**⇒ 因此真因仍是"倾斜缺直接观测"** ✓，机制更精确为：
**"恒倾角 + 零加速度 ⇒ 倾斜不可观测 ⇒ 冻结"** ✓✓

**⇒ 下一步**：核对参照在"零加速度段"如何维持倾斜可观测（其 `AccelBias` 默认启用 ✓
但零加速度下 b_a 与倾斜亦近简并 ✗）；若参照同样接受冻结 ⇒ 场景/判据应改为含**持续激励** ✓，
或启用其**显式可选**的重力辅助 ✓（`ImuCtrl::GravityVector` ✓，仅 |a| 小时启用 ✓ 正补此段 ✓✓）
