...
> ## ⚠️ 本文是**有日期的快照**，部分结论已被后续工作超越（2026-09-21 补注）
>
> 仍然有效的部分（当时定位的根本性问题）：
> - 时钟对齐机制 L1~L3、EKF 时间基用实测周期（§4.3/4.4）
> - vperiph 推流时间基（§4.5）
>
> **已被超越 / 已过期的部分**：
> - §2 的“两个 60s 测试 FAIL”、§4.6 的“x_hover_noise 残余极限环”：
>   当时未修；**它们现在的定位已完全改写** —— 见
>   `docs/stage4-outer-loop-findings.md` P17（及其 **归因修正**：控制拍率其实达
>   249.7Hz／余量 74%）。**该机制现已定位并修复（2026-09-21）**：不是闭环裕度问题，
>   而是**仿真双时基相位自由漂移**的测量假象（控制拍周期均值 4.0000ms 但真抖
>   std 0.84ms，物理按固定 4ms 步进 ⇒ PWM 回读相位随机）；`clock::run_one_control_tick`
>   锁相修复后 **`x_hover_noise` 已通过 ✓**（10.26°／13.02°，闸 15°）。
>   详见 `docs/test-roadmap.md`「解除阻塞的过程」。
> - 文中若出现“固件时间慢 N 倍/30Hz”：那是 `28bb0c5` 前旧标定的遗留，现已 1:1。
> - **本文不再是任何结论的权威来源**；以 `docs/test-roadmap.md` 与各 `stage*-findings.md` 为准。
...
**前置修复已落地（本节）**：把任务栈缓冲集中到独立 `.app_stacks` 段，测试可见全局放到
**固定 VMA `0x2000A000`** 的输出段（`joc-rtos-app-sdk` `app.ld`）——栈容量调整不再移动
全局。地址整体 **+0x1100**，8 个测试文件的硬编码 `.bss` 地址同步平移。

**速率低通已重上并验证**（`rate_lpf_tau=0.02s`）：`x_hover_noise` `max|roll|=9.55°`、
`max|pitch|=12.31°`（都进 15°）；`x_env_motion` 4/4、`x_vperiph` 3/3 无回归。

**残余判定与收口（按语义重标）**：原「水平漂移 5.00m」并非 EKF 偏置——诊断显示
EKF 位置/速度**精确跟随真值**，漂移的是**机体本身**（50s 自由漂移 5~20m，且每次随
噪声种子不同）。根因：解锁后是 **ALT_HOLD**，`rate_mode_xy` **旁路位置外环** → 水平
位置是自由积分，加水平加计零偏（EKF 只估计垂向 `x[9]`）后无界漂移。所以 `horiz<N`
的位置断言口径本就错误（位置在 ALT_HOLD 下无界）。改成断言姿态<15°、高度<3m、
**水平速度<1.5m/s**（发散才是问题）。`x_hover_noise` **通过**。
（尝试置 LOITER 启用位置保持 → 位置外环在逼真噪声下失稳，max|roll|=180°，另立待办 a3。）
