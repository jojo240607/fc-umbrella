...
- [ ] **44s 发散修复**：真因已定位到**姿态控制环符号/坐标系不一致（正反馈）**（§4.1
      候选 (a) 实验）——不是锚定、也不是陀螺。下一步逐轴核对
      `plant.rs::read_sensors`(omega_fc 反射) ↔ `flyctrl attitude_rates`/`x4_mix` ↔
      `plant.rs::step`(tau_body 反射)。
